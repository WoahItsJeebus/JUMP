...
class MinesweeperApp {
    __New() {
        ...
        ; Create the window (hidden) so we have a real HWND/client size to work with.
        this.Gui.Show("Hide")
    ...
    ApplyIdealSize() {
        ...
        ; Force the window to the exact grid size (AutoSize won't reliably shrink).
        ...
        ; Layout header against the (new) client width.
        this.LayoutHeader(sz.W)
    }
    ...
    OnSize(guiObj, MinMax, Width, Height) {
        ...
        ; Avoid re-entrancy if we resize programmatically.
        if this._InLayout
            return

        this._InLayout := true
        try this.LayoutHeader(Width)
